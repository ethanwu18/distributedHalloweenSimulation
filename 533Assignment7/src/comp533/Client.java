...
public class Client extends AnAbstractSimulationParametersBean implements ClientInterface, GIPCclientInt, SocketChannelConnectListener, SocketChannelWriteListener, SocketChannelReadListener {
	...
	protected Client(int aServerPort, ServerInterface server, IPCMechanism check) {
		...
		Thread cliRead = new Thread(new ClientReadThread(this.readQueue));
	...
	public void broadcast(String s) {
		...
		util.trace.port.consensus.ProposalLearnedNotificationReceived.newCase(this, util.trace.port.consensus.communication.CommunicationStateNames.COMMAND,-1, s);
		util.trace.port.consensus.ProposedStateSet.newCase(this, util.trace.port.consensus.communication.CommunicationStateNames.COMMAND,-1, s);
		if(getConsensusAlgorithm() == ConsensusAlgorithm.CENTRALIZED_SYNCHRONOUS && s == null) s = "";
	...
	public void init (String[] args) {
		setTracing();
		processArgs(args);
		listener = new CouplerListener(ClientArgsProcessor.getClientName(args), server, this);
		commandP1 = createSimulation(Simulation1.SIMULATION1_PREFIX);	
		commandP1.addPropertyChangeListener(listener);
		
		//add meta state replica
	}
	...
	public void ipcMechanism(IPCMechanism newValue) {
		if(getConsensusAlgorithm() == ConsensusAlgorithm.CENTRALIZED_SYNCHRONOUS) {
			...
			if (isBroadcastMetaState()) {
				...
				//find ipc object to add listener to to broadcast
			}
		} else {
		super.ipcMechanism(newValue);
		if (isBroadcastMetaState()) {
	...
	//	System.out.println();
	//	System.out.println("Consensus Algorithm Changed");
		if(!isAtomicBroadcast()) atomicBroadcast(true);
		if (isBroadcastMetaState()) {
			try {
				util.trace.port.consensus.ProposalMade.newCase(this, util.trace.port.consensus.communication.CommunicationStateNames.BROADCAST_MODE, -1, conAlg);
				server.broadcastCA(name, conAlg);
				util.trace.port.consensus.RemoteProposeRequestSent.newCase(this, util.trace.port.consensus.communication.CommunicationStateNames.BROADCAST_MODE, -1, conAlg);
	...
	public void broadcastGIPC(IPCMechanism str) {
		// TODO Auto-generated method stub
		util.trace.port.consensus.ProposalLearnedNotificationReceived.newCase(this, util.trace.port.consensus.communication.CommunicationStateNames.IPC_MECHANISM,-1, str);
		//util.misc.ThreadSupport.sleep(this.getDelay());
		//util.trace.port.consensus.ProposalAcceptRequestReceived.newCase(this, util.trace.port.consensus.communication.CommunicationStateNames.IPC_MECHANISM,-1, str); //potentially command

		super.ipcMechanism(str);
		util.trace.port.consensus.ProposedStateSet.newCase(this, util.trace.port.consensus.communication.CommunicationStateNames.IPC_MECHANISM,-1, str);
	}

	@Override
	public void broadcastMS(boolean newValue) {
		// TODO Auto-generated method stub
		util.trace.port.consensus.ProposalLearnedNotificationReceived.newCase(this, util.trace.port.consensus.communication.CommunicationStateNames.BROADCAST_MODE,-1, newValue);

		setBroadcastMetaState(newValue);
		util.trace.port.consensus.ProposedStateSet.newCase(this, util.trace.port.consensus.communication.CommunicationStateNames.BROADCAST_MODE,-1, newValue);
	...
	public void connected(SocketChannel aSocketChannel) {
		nioManager.addReadListener(aSocketChannel, this); // a socket channel is the server channel
	}
	@Override
	...
	public void broadcastAtomic(boolean isAtomic) {
		util.trace.port.consensus.ProposalLearnedNotificationReceived.newCase(this, util.trace.port.consensus.communication.CommunicationStateNames.BROADCAST_MODE,-1, isAtomic);

		setAtomicBroadcast(isAtomic);
		commandP1.setConnectedToSimulation(!isAtomicBroadcast());

		util.trace.port.consensus.ProposedStateSet.newCase(this, util.trace.port.consensus.communication.CommunicationStateNames.BROADCAST_MODE,-1, isAtomic);
	...
	public void broadcastCA(ConsensusAlgorithm conAlg) {
		// TODO Auto-generated method stub
		util.trace.port.consensus.ProposalLearnedNotificationReceived.newCase(this, util.trace.port.consensus.communication.CommunicationStateNames.BROADCAST_MODE,-1, conAlg);

		this.setConsensusAlgorithm(conAlg);
		
		util.trace.port.consensus.ProposedStateSet.newCase(this, util.trace.port.consensus.communication.CommunicationStateNames.BROADCAST_MODE,-1, conAlg);

	}


	@Override
	public void accept(String clientInt, String command) {
		//System.out.println("accept recieved");
		util.trace.port.consensus.ProposalAcceptRequestReceived.newCase(this, util.trace.port.consensus.communication.CommunicationStateNames.COMMAND,-1, null); //potentially command
		util.trace.port.consensus.ProposalAcceptedNotificationSent.newCase(this, util.trace.port.consensus.communication.CommunicationStateNames.COMMAND,-1, null, ProposalFeedbackKind.SUCCESS); //potentially command
		//might be problem with remote? GIPC? idk why i need try catch here
			try {
				server.setAccept(ProposalFeedbackKind.SUCCESS);
			} catch (RemoteException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
